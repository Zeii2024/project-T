<template>
	<view class="muli-tags" @click="handleClick">

		<!-- <view class="muli-tags" @click="handleClick" style="background-color: snow;"> -->
		<span ref="tag" class="tag" v-for="(tag, index) in tags" :key="`${index}-${tag}`" :data-tag-index="index">
			<i class="label">{{tag}}</i>
		</span>
		<input ref="inputElement" v-model="current" type="text" class="input" :size="current.length || 1"
			@keyup.enter="add" @keydown.delete="del" @blur="add" @click.stop @keydown.up="selectItem($event, 'before')"
	...
	export default {
		...
		computed: {
			relativeTagOpts() {
				...
				console.log("relativeTagOpts######当前输入值为: ", curInputVal)
				if (!curInputVal) return [];
				console.log("relativeTagOpts######若当前输入值，列表中包含，就会筛选出来", this.relativeOpts.filter(opt => opt.includes(
					curInputVal)))
		...
		methods: {
			focus() {
				console.log("进入focus()方法**********************")
				console.log("######", this.$refs) // .type text
				// /* this.$refs---[object] */
				// this.$nextTick(() => {
