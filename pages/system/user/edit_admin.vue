<template>
	<view class="uni-container">
		<uni-forms ref="form" v-model="formData" :rules="rules" validateTrigger="bind" @submit="submit">
			...
			<!-- <uni-forms-item name="position" label="职位列表" class="flex-center-x">
					<uni-data-checkbox multiple :localdata="position" v-model="formData.role" />
				</uni-forms-item> -->
			<uni-forms-item name="position" label="职位列表" labelWidth="100" class="flex-center-x">
				<uni-data-checkbox multiple :localdata="position" v-model="formData.position" />
				<span class="link-btn" @click="gotoPositionAdd">修改</span>
			</uni-forms-item>
			<!-- <uni-forms-item name="authorizedApp" label="可登录应用">
				<view class="uni-forms-item-flex-center-x">
					<uni-data-checkbox :multiple="true" v-model="formData.authorizedApp"
						:localdata="appList"></uni-data-checkbox>
					<span class="link-btn" @click="gotoAppList">管理</span>
				</view>
				<view v-if="formDataId === userId" class="uni-form-item-tips">
					当前有未添加的应用{{ unknownAppidsCom }}，建议点击右侧管理进行添加</view>
			</uni-forms-item> -->

			<uni-forms-item name="work_cost" label="工时成本">
				<uni-easyinput v-model="formData.work_cost" :clearable="false" placeholder="0" />
			</uni-forms-item>
			<uni-forms-item name="permission" label="权限" labelWidth="100" class="flex-center-x">
				<uni-data-checkbox multiple :localdata="permission" v-model="formData.permission" />
				<span class="link-btn" @click="gotoPermissionAdd">修改</span>
			</uni-forms-item>
				...
				db.collection(dbCollectionName)
					...
					.then((res) => {
						...
						if (data) {
							...
							this.position = data.position.map(item => {
								return {
									name: item,
									text: item
								}
							...
							this.permission = data.permission.map(item => {
								return {
									name: item,
									text: item
								}
